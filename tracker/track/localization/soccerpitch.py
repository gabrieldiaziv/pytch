...
class SoccerPitch:
    ...
    def get_line_vertices(self, line_name):
        """
        For lines belonging to the pitch lawn plane returns its vertices
        """
        # ensure line in football pitch plane
        if line_name in self.line_extremities.keys() and \
                "post" not in line_name and \
    ...
    def x_to_image(self, dimension, value):
        return int(dimension*(value/self.PITCH_LENGTH) + dimension/2)
    
    def y_to_image(self, dimension, value):
        return int(dimension*(value/self.PITCH_WIDTH) + dimension/2)
